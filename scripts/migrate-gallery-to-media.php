...
/**
 * @file
 * Migrate field_telikert_kep images to field_gallery_media (Media entities).
 *
 * Usage: drush scr scripts/migrate-gallery-to-media.php
 */
...
foreach ($nids as $nid) {
  ...
  if (!$node->get('field_gallery_media')->isEmpty()) {
    echo "  - Already has media gallery, skipping.\n";
    $skipped++;
    continue;
  }

  // Get images from old field.
  $old_images = $node->get('field_telikert_kep')->getValue();
  if (empty($old_images)) {
    echo "  - No images in field_telikert_kep, skipping.\n";
    $skipped++;
    continue;
  }
